...
{
	public abstract partial class WebSocketTransport : IRSocketTransport
    {
        private readonly WebSocketOptions socketOptions;
        private ILogger<WebSocketTransport> logger;
        private readonly IDuplexPipe front;
        private readonly IDuplexPipe back;
        ...
        protected ValueTask Running
        {
            get; 
            set;
        }

        protected WebSocketsTransport Transport
        {
            get;
        }
            ...
            PipeOptions inputPipeOptions = default)
        {
            ...
            socketOptions = WebSocketsTransport.DefaultWebSocketOptions;
            (front, back) = DuplexPipe.CreatePair(outputPipeOptions, inputPipeOptions);
            Transport = new WebSocketsTransport(back, socketOptions, logger);
        }

        public ValueTask StartAsync(CancellationToken cancellationToken = default)
        {
			//NOTE: should be re-entrancy
            Running = ProcessRequestAsync(cancellationToken);
            return new ValueTask(Task.CompletedTask);
        }

        public virtual Task StopAsync() => Task.CompletedTask;

        protected abstract ValueTask ProcessRequestAsync(CancellationToken cancellationToken);
    }
}
	...
	/*partial class WebSocketsTransport
    {
        ...
        public class WebSocketManager     //https://github.com/aspnet/AspNetCore/blob/master/src/Http/Http/src/Internal/DefaultWebSocketManager.cs
        {
            ...
            public WebSocketManager(WebSocketTransport transport, CancellationToken cancel = default)
            {
                Transport = transport;
                Cancel = cancel;
            }
		...
		public class HttpContext              //https://github.com/aspnet/AspNetCore/blob/master/src/Http/Http.Abstractions/src/HttpContext.cs
        {
            ...
            public HttpContext(WebSocketTransport transport, CancellationToken cancel = default)
            {
                WebSockets = new WebSocketManager(transport, cancel);
            }
